...
class ConfigParser
{
	...
	private:
		...
		void processLocation(const std::string& key, const std::string& value, ServerConfig& config, RouteConfig& route);
		void processRedirect(const std::string& key, const std::string& value, ServerConfig& config, RouteConfig& route);
};
